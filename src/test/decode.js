...
function decodeFile(vttPath,jsonPath,index) {
    ...
    let bufferStr=arrayToStr(buf);
    let PageArrays=strToArray(bufferStr,index,CONF);
    ...
    let BPageStrs=decodeBPage(IPageStr,PPageStrs,PageArrays['BPageArray'],value,CONF,index);
    let HTML=strToHtml(IPageStr,PPageStrs,BPageStrs,CONF);
    fs.writeFileSync(`${vttPath.replace('vtt','txt')}`, HTML);
...
decodeFile(`${rootPath}build/kai-1.0.0/kai_w1020_h300_f10/zip/kai_w1020_h300_f10_z2_b2/kai_w1020_h300_f10_z2_b2_1.vtt`,`${rootPath}build/kai-1.0.0/kai_w1020_h300_f10/zip/kai_w1020_h300_f10_z2_b2/config.json`,0);